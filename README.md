...
- **Security Group**: `sg-02c5fbd7d50f084d1`
...
| 명령어 | 설명 |
|--------|------|
| `terraform init` | Terraform 초기화 및 Provider 다운로드 |
| `terraform init -upgrade` | Provider를 최신 버전으로 업그레이드 |
...
| `terraform apply -auto-approve` | 확인 없이 바로 적용 (주의!) |
...
| `terraform state show <리소스>` | 특정 리소스 상세 정보 |
| `terraform state pull` | 원격 상태 파일 가져오기 |
| `terraform refresh` | 상태 파일 새로고침 |

### 출력 및 정보

| 명령어 | 설명 |
|--------|------|
| `terraform output` | 모든 출력 값 표시 |
| `terraform output <이름>` | 특정 출력 값 표시 |
